...
def eval_model(model, dataloader, confidence_threshold=0.5):
    model = model.eval()
    answer_list = []
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Running Evaluation'):
            ...
            start_probs = F.softmax(out_start, dim=1)
            end_probs = F.softmax(out_end, dim=1)
            ...
            if confidence >= confidence_threshold:
                predicted_answer = tokenizerFast.convert_tokens_to_string(tokenizerFast.convert_ids_to_tokens(input_ids[0][start_pred:end_pred]))
                true_answer_start = batch['start_positions'].item()
                true_answer_end = batch['end_positions'].item()
                true_answer = tokenizerFast.convert_tokens_to_string(tokenizerFast.convert_ids_to_tokens(input_ids[0][true_answer_start:true_answer_end]))
                answer_list.append([predicted_answer, true_answer])
            else:
                answer_list.append(["", ""]) 
...
for epoch in range(5):
    ...
    answer_list = eval_model(model, valid_data_loader, confidence_threshold=0.3)
    pred_answers=[]
    true_answers=[]
...
print('WER of BOSS model ',wer_list)
